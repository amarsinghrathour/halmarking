...
/**
 * Description of RecentActivityService
 *
 * @author singh
 */
...
class RecentActivityService {
    ...
    public static function save($request) {
        ...
        try {
            ...
            if ($request->hasFile('image')) {
                $imageDb = uploadImage($request->file('image'), 'activity', 'activity');
            }
            ...
            $upcomingEvent = new RecentActivity;
            $upcomingEvent->title = $title;
            $upcomingEvent->url = $url;
            $upcomingEvent->activity_date = $activity_date;
    ...
    public static function update($request) {
        ...
        try {
            ...
            $upcomingEvent = RecentActivity::find($id);
            $upcomingEvent->title = $title;
    ...
    protected static function create_slug($string) {
        ...
        do {
            ...
            $data = RecentActivity::where('url', $slug)->count();
            if ($data > 0) {
                $string = $slug . '-' . $data;
            }
        } while ($data > 0);
