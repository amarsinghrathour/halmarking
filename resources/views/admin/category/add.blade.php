            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('admin.category.save') }}" method="POST" id="saveProduct"  enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                            <div class="col-md-6 form-group">
                                                <label for="name">Parent</label>
                                                <select class="form-control select2" style="width: 90%" name="parent_id" required="">
                                                    <option value="0" selected="">Leave as Parent</option>
                                                    @foreach($categoryList as $value)
                                                    <option value="{{$value->id}}" @if(old('parent_id') == $value->id) selected="" @endif>{{$value->title}}</option>
                                                    @endforeach
                                                </select>
                                                @error('parent_id')
                                ...
                                <div class="col-md-6 form-group">
                                    <label for="name">Image(350 X 460) <span class="text-danger">*</span></label>
                                    <input type="file" name="image" class="form-control @error('image') is-invalid @enderror" onchange="previewImage(this);" required="">
                                    <span>(Note: Allow Type: jpg,jpeg,png,bmp,tiff Max Size: 2mb, width: 350px, Height: 460px)</span>
                                    @error('image')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                            ...
                            <div class="box-footer">
                                <div class="col-md-12" style="text-align: center">
                                    <a href="{{route('admin.category.list')}}" class="btn btn-primary"><i class="fa fa-backward"></i> {{__('Back')}}</a>
