  ...
  <div class="preloader flex-column justify-content-center align-items-center">
    ...
    <div class="wrapper">
                ...
                if(null !== $myTopMenu){
                    foreach ($myTopMenu as $topMenuList){
                        if($topMenuList->url === "#"){
                            ...
                            $topMenuUrl = url(config('app.admin_prefix').$topMenuList->url);
                        }
                        if($topMenuUrl === "#"){
                            ?>
                <li class="nav-item dropdown">
                    <a id="dropdownSubMenu1" href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="nav-link dropdown-toggle">
                        <i class="nav-icon {{ $topMenuList->icon }}"></i>
                        {{$topMenuList->name}}
                    </a>
                    <ul aria-labelledby="dropdownSubMenu1" class="dropdown-menu border-0 shadow">
                        ...
                        foreach ($topSubMenu as $topSubMenuList){
                            ?>
                        <li>
                            <a href="{{url(config('app.admin_prefix').$topSubMenuList->url)}}" class="dropdown-item">
                                <i class="nav-icon {{ $topSubMenuList->icon }}"></i>
                                {{$topSubMenuList->name}}
                            </a>
                        </li>
                            <?php
        ...
        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right" style="left: inherit; right: 0px;">
          ...
          <div class="dropdown-divider"></div>
          <a class="dropdown-item" href="#" title="Log Out" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" role="button">
                ...
                <ul class="nav nav-pills nav-sidebar flex-column nav-child-indent" data-widget="treeview" role="menu" data-accordion="false">
                    ...
                    if(null !== $myMenu){
                        ...
                        if ($_SERVER["HTTP_HOST"] == "localhost" or strpos($_SERVER["HTTP_HOST"], "192.168") !== false) {
                            if (isset($exp[3])) {
                                $navigation_controller = $exp[3];
                            }
                            if (isset($exp[4])) {
                                $navigation_action = $exp[4];
                            }
                        } else {
                            if (isset($exp[2])) {
                                $navigation_controller = $exp[2];
                            }
                            if (isset($exp[3])) {
                                $navigation_action = $exp[3];
                            }
                        }
                        ...
                        foreach ($myMenu as $menuList){
                            if($menuList->url === "#"){
                                ...
                                $menuUrl = url(config('app.admin_prefix').$menuList->url);
                            }
                            $menu2 = App\Models\Menu::getLeftMenu($menuList->id);
                            $active=""; $menuOpen="";
                            foreach ($menu2 as $menuL2){
                                if(App\Models\Menu::checkForActiveController($menuL2->url.'/',$navigation_controller)){
                                    $active="active";
                                    $menuOpen = "menu-open";
                                    break;
                                }
                                        ...
                                        if(count($menu2) > 0){
                                            foreach ($menu2 as $menuL2){
                                                if($menuL2->url == "#"){
                                                    ...
                                                    $menuUrl = url(config('app.admin_prefix').$menuL2->url);
                                                }
                                                $menu3 = App\Models\Menu::getLeftMenu($menuL2->id);
                                                $active=""; $menuOpen="";
                                                foreach ($menu3 as $menuL3){
                                                    if(App\Models\Menu::checkForActiveController($menuL3->url.'/',$navigation_controller)){
                                                        $active="active";
                                                        $menuOpen = "menu-open";
                                                        break;
                                                    }
                                                          ...
                                                          if(count($menu3) > 0){
                                                              foreach ($menu3 as $menuL3){
                                                                  if($menuL3->url == "#"){
                                                                      ...
                                                                      $menuUrl = url(config('app.admin_prefix').$menuL3->url);
                                                                  }
                                                                  $active="";
                                                                  if('1' == $menuL3->id){
                                                                      $active="active";
                                                ...
                                                <?php
                                                    ...
                                                    $active="";
                                                    if(App\Models\Menu::checkForActiveAction($menuL2->url.'/',$navigation_action)){
                                                        $active="active";
                                                    }?>
                                                    <li class="nav-item">
                                                        <a href="{{ $menuUrl }}" class="nav-link {{ $active }}">
                            ...
                            <?php
                                ...
                                $active="";
                                if(App\Models\Menu::checkForActiveController($menuList->url.'/',$navigation_controller)){
                                    $active="active";
                                }
                                ?>
                                <li class="nav-item">
                  ...
                  <li class="nav-item">
                    ...
                    <ul class="nav nav-treeview">
                      <li class="nav-item">
                        <a href="{{route('lock-screen')}}" class="nav-link">
                          <i class="nav-icon fas fa-lock"></i>
                          <p>Lock Screen</p>
                        </a>
                      </li>
                      <li class="nav-item">
                         <a class="nav-link" href="#" onclick="event.preventDefault(); document.getElementById('shut-down').submit();" role="button">
                          <i class="nav-icon fas fa-power-off"></i>
                          <p>Log Out</p>
                         </a>
                         <form id="shut-down" action="{{ route('admin.logout') }}" method="POST" style="display: none;">
                           @csrf
                         </form>
                      </li>
                    </ul>
