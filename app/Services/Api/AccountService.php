...
class AccountService extends Model
{
    ...
    public static function getSlots($request)
    {
        ...
        if (auth()->user()->id) {
            ...
            Log::debug(__CLASS__."::".__FUNCTION__."called  with customer id ".auth()->user()->id);
             ...
             try{
                $slots = SlotDate::where('status','ACTIVE')->whereHas('slot_time', function ($query)
                     {
                          $query->where('status', '=', 'ACTIVE');
                     })->with(['slot_time' => function($q) {
                        $q->where('status', '=', 'ACTIVE');
                    }])->orderBy('id','desc')->get();
                if(count($slots) > 0){
                        return returnResponse("Slots Returned", HttpStatus::HTTP_OK, HttpStatus::HTTP_SUCCESS,$slots);
    ...
    public static function getAppointments($request)
    {
        ...
        if (auth()->user()->id) {
            ...
            Log::debug(__CLASS__."::".__FUNCTION__."called  with customer id ".auth()->user()->id);
             ...
             try{
                $slots = Appointment::where('customer_id',auth()->user()->id)->where('status','!=','DELETED')->with('history')->orderBy('id','desc')->get();
                if(count($slots) > 0){
                        return returnResponse("Appointments Returned", HttpStatus::HTTP_OK, HttpStatus::HTTP_SUCCESS,$slots);
                    
                }  else {
                        Log::error("Error Occured at Appointments getting... ");
                        return returnResponse("Data not available !",HttpStatus::HTTP_NOT_FOUND, HttpStatus::HTTP_WARNING);
                    }
            ...
            Log::error("Error Occured at get Appointments ");
            return returnResponse("Some Error Occured please try again !");
        } 
        return returnResponse(HttpStatus::$text[HttpStatus::HTTP_UNAUTHORIZED], HttpStatus::HTTP_UNAUTHORIZED);
    }
    ...
    public static function bookAppointment($request)
    {
        ...
        $unplugged = htmlspecialchars(strip_tags($request->unplugged));
        
        $fresh_track = $request->input('fresh_track');
        
        $karaoke = $request->input('karaoke');
        ...
        if(auth()->user()->id) {
            ...
            try{
                ...
                $appointment->unplugged = $unplugged;
                $appointment->karaoke = $karaoke;
                $appointment->fresh_track = $fresh_track;
                $appointment->dubbings = $dubbings;
